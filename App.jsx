...
function toDb(item) {
  return { id: item.id, title: item.title, category: item.category, city: item.city || "", state: item.state || "", country: item.country || "", your_rating: item.yourRating || 0, her_rating: item.herRating || 0, cost: item.cost || 0, trip_length: item.tripLength || "", kids_trip: item.kidsTrip || false, notes: item.notes || "", status: item.status || "active", sort_order: item.sortOrder || 0 };
}
function fromDb(row) {
  return { id: row.id, title: row.title, category: row.category, city: row.city || "", state: row.state || "", country: row.country || "", yourRating: row.your_rating || 0, herRating: row.her_rating || 0, cost: row.cost || 0, tripLength: row.trip_length || "", kidsTrip: row.kids_trip || false, notes: row.notes || "", status: row.status || "active", sortOrder: row.sort_order || 0 };
}
function genId() { return Math.random().toString(36).slice(2, 10); }

const CATEGORIES = ["City", "Museum", "Restaurant", "Hotel", "Experience", "Nature", "Entertainment", "Other"];
const SORT_OPTIONS = ["Priority Score", "Your Rating", "Her Rating", "Cost: Low to High", "Cost: High to Low", "Trip Length", "Name", "Country"];
const COST_LEVELS = [1, 2, 3, 4, 5];
const TRIP_LENGTHS = ["Afternoon", "Day trip", "Long weekend", "Vacation"];
const TRIP_COLORS = { "Afternoon": "#185FA5", "Day trip": "#1D9E75", "Long weekend": "#BA7517", "Vacation": "#D4537E" };
...
function TripBadge({ tripLength }) {
  if (!tripLength) return null;
  const bg = TRIP_COLORS[tripLength] || "#888780";
  return <span style={{ background: bg, color: "#fff", borderRadius: 10, padding: "2px 9px", fontSize: 11, fontWeight: 500 }}>{tripLength}</span>;
}
...
function CostPicker({ value, onChange }) {
  ...
  return (
    <div style={{ display: "flex", gap: 6 }}>
      {COST_LEVELS.map(function(i) {
        return (
          <button key={i} onClick={function() { onChange(value === i ? 0 : i); }}
            style={{ flex: 1, borderRadius: 10, padding: "10px 4px", border: value === i ? "2px solid #1D9E75" : "2px solid #d0d0d8", background: value === i ? "#EAF3DE" : "#fff", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, fontFamily: "inherit" }}>
...
function TripLengthPicker({ value, onChange }) {
  return (
    <div style={{ display: "flex", gap: 6 }}>
      {TRIP_LENGTHS.map(function(t) {
        const active = value === t;
        const col = TRIP_COLORS[t] || "#888780";
        return (
          <button key={t} onClick={function() { onChange(active ? "" : t); }}
            style={{ flex: 1, borderRadius: 10, padding: "10px 4px", border: active ? "2px solid " + col : "2px solid #d0d0d8", background: active ? "#f0f7ff" : "#fff", cursor: "pointer", fontSize: 11, fontWeight: active ? 600 : 400, color: active ? col : "#888780", fontFamily: "inherit" }}>
            {t}
          </button>
        );
      })}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      ...
      <div style={{ display: "flex", gap: 2 }}>
        {[1, 2, 3, 4, 5].map(function(i) {
          return <span key={i} onClick={function() { onChange && onChange(i); }} style={{ cursor: onChange ? "pointer" : "default", fontSize: 28, color: i <= value ? "#EF9F27" : "#d0d0d8", lineHeight: 1 }}>★</span>;
        })}
      </div>
    </div>
  );
...
function ItemForm({ item, onSave, onClose, saving, error }) {
  const blank = { title: "", category: "Experience", city: "", state: "", country: "", yourRating: 0, herRating: 0, cost: 0, tripLength: "", kidsTrip: false, notes: "", status: "active" };
  ...
  return (
    <Panel title={item ? "Edit item" : "New destination"} onClose={onClose}>
      ...
      <SectionCard><Field label="Destination name"><input placeholder="e.g. Tsukiji Fish Market" value={form.title} onChange={function(e) { set("title", e.target.value); }} style={inputStyle} /></Field></SectionCard>
      <SectionCard>
        <label style={labelStyle}>Category</label>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 6 }}>
          {CATEGORIES.map(function(c) { return <button key={c} onClick={function() { set("category", c); }} style={{ background: form.category === c ? CAT_COLORS[c] : "#f5f5f7", color: form.category === c ? "#fff" : "#888780", border: form.category === c ? "none" : "0.5px solid #d0d0d8", borderRadius: 10, padding: "9px 4px", fontSize: 12, cursor: "pointer", fontWeight: form.category === c ? 500 : 400, fontFamily: "inherit" }}>{c}</button>; })}
        </div>
      </SectionCard>
      <SectionCard>
        <label style={labelStyle}>Location</label>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
          <Field label="City"><input placeholder="Tokyo" value={form.city} onChange={function(e) { set("city", e.target.value); }} style={inputStyle} /></Field>
          <Field label="State"><input placeholder="Optional" value={form.state} onChange={function(e) { set("state", e.target.value); }} style={inputStyle} /></Field>
          <Field label="Country"><input placeholder="Japan" value={form.country} onChange={function(e) { set("country", e.target.value); }} style={inputStyle} /></Field>
        </div>
      </SectionCard>
      <SectionCard>
        <label style={labelStyle}>Ratings</label>
        <div style={{ display: "flex", justifyContent: "space-around", paddingBottom: 12 }}>
          <StarRating value={form.yourRating} onChange={function(v) { set("yourRating", v); }} label="Steve" />
          <div style={{ width: "0.5px", background: "#e5e5e7" }} />
          <StarRating value={form.herRating} onChange={function(v) { set("herRating", v); }} label="Jenn" />
        </div>
      ...
      <SectionCard><label style={labelStyle}>Estimated cost</label><CostPicker value={form.cost} onChange={function(v) { set("cost", v); }} /></SectionCard>
      <SectionCard><label style={labelStyle}>Trip length</label><TripLengthPicker value={form.tripLength || ""} onChange={function(v) { set("tripLength", v); }} /></SectionCard>
      <SectionCard>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div onClick={function() { set("kidsTrip", !form.kidsTrip); }} style={{ width: 48, height: 28, borderRadius: 14, background: form.kidsTrip ? "#EF9F27" : "#d0d0d8", cursor: "pointer", position: "relative", flexShrink: 0 }}>
      ...
      <SectionCard><Field label="Notes, tips & itinerary"><textarea placeholder="Add details, links, tips..." value={form.notes} onChange={function(e) { set("notes", e.target.value); }} rows={4} style={{ ...inputStyle, resize: "vertical", lineHeight: 1.6 }} /></Field></SectionCard>
      {item && <SectionCard><div style={{ display: "flex", alignItems: "center", gap: 10 }}><input type="checkbox" id="done" checked={form.status === "complete"} onChange={function(e) { set("status", e.target.checked ? "complete" : "active"); }} style={{ width: 18, height: 18, cursor: "pointer" }} /><label htmlFor="done" style={{ fontSize: 15, color: "#1c1c1e", cursor: "pointer" }}>Mark as completed</label></div></SectionCard>}
      <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
        <GhostBtn onClick={onClose} disabled={saving}>Cancel</GhostBtn>
        <PrimaryBtn onClick={function() { if (form.title.trim()) onSave(form); }} disabled={!form.title.trim() || saving}>{saving ? "Saving…" : item ? "Save changes" : "Add to list"}</PrimaryBtn>
      </div>
    </Panel>
  );
}
  ...
  return (
    <Panel title="Bulk import" onClose={onClose}>
      ...
      <SectionCard>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 6 }}>
          {CATEGORIES.map(function(c) { return <button key={c} onClick={function() { setCategory(c); }} style={{ background: category === c ? CAT_COLORS[c] : "#f5f5f7", color: category === c ? "#fff" : "#888780", border: category === c ? "none" : "0.5px solid #d0d0d8", borderRadius: 10, padding: "9px 4px", fontSize: 12, cursor: "pointer", fontWeight: category === c ? 500 : 400, fontFamily: "inherit" }}>{c}</button>; })}
        </div>
      </SectionCard>
      <SectionCard>
        <label style={labelStyle}>Paste your list</label>
        <textarea value={text} onChange={function(e) { setText(e.target.value); }} placeholder={"Eiffel Tower, Paris, , France\nNobu, New York, NY, USA\nKyoto"} rows={8} style={{ ...inputStyle, fontFamily: "monospace", fontSize: 13, resize: "vertical" }} />
        <p style={{ fontSize: 12, color: "#888780", margin: "8px 0 0" }}>{lines.length} item{lines.length !== 1 ? "s" : ""} detected</p>
      </SectionCard>
      <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
        <GhostBtn onClick={onClose} disabled={saving}>Cancel</GhostBtn>
        <PrimaryBtn onClick={function() { onImport(lines.map(function(line) { const p = line.split(",").map(function(s) { return s.trim(); }); return { id: genId(), title: p[0] || line, city: p[1] || "", state: p[2] || "", country: p[3] || "", category: category, yourRating: 0, herRating: 0, cost: 0, tripLength: "", kidsTrip: false, notes: "", status: "active" }; })); }} disabled={!lines.length || saving}>{saving ? "Saving…" : "Import " + lines.length + " items"}</PrimaryBtn>
      </div>
    </Panel>
  );
}
...
function ItemDetail({ item, onSave, onDelete, onClose, saving, error }) {
  ...
  if (editing) return <ItemForm item={item} onSave={async function(v) { await onSave(v); setEditing(false); }} onClose={function() { setEditing(false); }} saving={saving} error={error} />;
    ...
    <Panel title={item.title} onClose={onClose}>
      ...
      <SectionCard>
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: loc ? 6 : 0 }}>
          ...
          {item.tripLength && <TripBadge tripLength={item.tripLength} />}
        </div>
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
        <GhostBtn danger onClick={function() { if (confirm("Delete this item?")) onDelete(item.id); }} disabled={saving}>Delete</GhostBtn>
        <GhostBtn onClick={function() { onSave({ ...item, status: item.status === "complete" ? "active" : "complete" }); }} disabled={saving}>{item.status === "complete" ? "Mark active" : "Mark done"}</GhostBtn>
        <PrimaryBtn onClick={function() { setEditing(true); }} disabled={saving}>Edit</PrimaryBtn>
      </div>
    </Panel>
  );
}
...
export default function App() {
  ...
  const [filterTripLength, setFilterTripLength] = useState("All");
  ...
  useEffect(function() {
    dbLoad().then(function(rows) { setItems(rows); setLoading(false); }).catch(function(e) { setError("Could not load: " + e.message); setLoading(false); });
  }, []);

  const addItem = useCallback(async function(form) {
    setSaving(true); setError(null);
    try {
      const newItem = { ...form, id: genId(), sortOrder: items.length };
      await dbInsert([toDb(newItem)]);
      setItems(function(prev) { return [newItem, ...prev]; });
      setPanel(null);
    } catch(e) { setError("Failed to save: " + e.message); }
    setSaving(false);
  }, [items.length]);

  const updateItem = useCallback(async function(form) {
    setSaving(true); setError(null);
    try {
      await dbUpdate(form.id, toDb(form));
      setItems(function(prev) { return prev.map(function(i) { return i.id === form.id ? { ...form } : i; }); });
      setPanel({ item: { ...form } });
    } catch(e) { setError("Failed to update: " + e.message); }
    setSaving(false);
  }, []);

  const deleteItem = useCallback(async function(id) {
    setSaving(true); setError(null);
    try {
      await dbDelete(id);
  ...
  const importItems = useCallback(async function(newItems) {
    setSaving(true); setError(null);
    try {
      const withOrder = newItems.map(function(item, i) { return { ...item, sortOrder: i }; });
      await dbInsert(withOrder.map(toDb));
      setItems(function(prev) { return [...withOrder, ...prev]; });
      setPanel(null);
  ...
  function saveOrder(orderedIds) {
    ...
    orderedIds.forEach(function(id, i) { dbUpdate(id, { sort_order: i }); });
  }

  const scored = useMemo(function() { return items.map(function(i) { return { ...i, score: i.yourRating + i.herRating }; }); }, [items]);

  const filtered = useMemo(function() {
    let l = scored;
    if (filterStatus !== "all") l = l.filter(function(i) { return i.status === filterStatus; });
    if (filterCat !== "All") l = l.filter(function(i) { return i.category === filterCat; });
    if (filterCost !== "All") l = l.filter(function(i) { return i.cost === parseInt(filterCost); });
    if (filterKids !== "All") l = l.filter(function(i) { return filterKids === "yes" ? i.kidsTrip : !i.kidsTrip; });
    if (filterTripLength !== "All") l = l.filter(function(i) { return i.tripLength === filterTripLength; });
    if (search.trim()) { const q = search.toLowerCase(); l = l.filter(function(i) { return i.title.toLowerCase().includes(q) || i.city.toLowerCase().includes(q) || i.country.toLowerCase().includes(q); }); }
    return l;
  }, [scored, filterCat, filterStatus, filterCost, filterKids, filterTripLength, search]);

  const sorted = useMemo(function() {
    if (manualOrder) { const m = Object.fromEntries(filtered.map(function(i) { return [i.id, i]; })); return [...manualOrder.filter(function(id) { return m[id]; }).map(function(id) { return m[id]; }), ...filtered.filter(function(i) { return !manualOrder.includes(i.id); })]; }
    const l = [...filtered];
    if (sortBy === "Priority Score") l.sort(function(a, b) { return b.score - a.score; });
    else if (sortBy === "Your Rating") l.sort(function(a, b) { return b.yourRating - a.yourRating; });
    else if (sortBy === "Her Rating") l.sort(function(a, b) { return b.herRating - a.herRating; });
    else if (sortBy === "Cost: Low to High") l.sort(function(a, b) { return (a.cost || 0) - (b.cost || 0); });
    else if (sortBy === "Cost: High to Low") l.sort(function(a, b) { return (b.cost || 0) - (a.cost || 0); });
    else if (sortBy === "Trip Length") l.sort(function(a, b) { return TRIP_LENGTHS.indexOf(a.tripLength || "") - TRIP_LENGTHS.indexOf(b.tripLength || ""); });
    else if (sortBy === "Name") l.sort(function(a, b) { return a.title.localeCompare(b.title); });
    else if (sortBy === "Country") l.sort(function(a, b) { return (a.country || "").localeCompare(b.country || ""); });
    return l;
  ...
  function handleDragEnd(e) {
    ...
    const ids = sorted.map(function(i) { return i.id; });
    const f = ids.indexOf(dragItem.current); const t = ids.indexOf(dragOver.current);
    const no = [...ids]; no.splice(f, 1); no.splice(t, 0, dragItem.current);
    saveOrder(no); dragItem.current = null; dragOver.current = null;
  }

  const isFiltered = search.trim() || filterCat !== "All" || filterStatus !== "active" || filterCost !== "All" || filterKids !== "All" || filterTripLength !== "All";
  const totalAll = items.length;
  const totalActive = items.filter(function(i) { return i.status === "active"; }).length;
  const totalDone = items.filter(function(i) { return i.status === "complete"; }).length;
    ...
    <div style={{ padding: "1rem 0", maxWidth: 720, margin: "0 auto" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.25rem" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={function() { setError(null); setPanel("bulk"); }} style={{ fontSize: 13, fontFamily: "inherit", background: "#fff", border: "0.5px solid #d0d0d8", borderRadius: 8, padding: "8px 14px", cursor: "pointer", color: "#1c1c1e" }}>Bulk import</button>
          <button onClick={function() { setError(null); setPanel("add"); }} style={{ background: "#185FA5", color: "#fff", border: "none", fontSize: 15, fontWeight: 500, padding: "10px 20px", borderRadius: 10, cursor: "pointer", fontFamily: "inherit" }}>+ Add item</button>
        </div>
      </div>

      {panel === "add" && <ItemForm onSave={addItem} onClose={function() { setPanel(null); setError(null); }} saving={saving} error={error} />}
      {panel === "bulk" && <BulkImport onImport={importItems} onClose={function() { setPanel(null); setError(null); }} saving={saving} error={error} />}
      {panel && panel.item && <ItemDetail item={panel.item} onSave={updateItem} onDelete={deleteItem} onClose={function() { setPanel(null); setError(null); }} saving={saving} error={error} />}

      {error && !panel && <div style={{ background: "#FCEBEB", color: "#A32D2D", borderRadius: 10, padding: "10px 14px", marginBottom: "1rem", fontSize: 14, display: "flex", justifyContent: "space-between", alignItems: "center" }}>{error}<button onClick={function() { setError(null); }} style={{ background: "none", border: "none", cursor: "pointer", color: "#A32D2D", fontSize: 18 }}>×</button></div>}

      <div style={{ display: "flex", gap: 6, marginBottom: "1rem" }}>
        {[{ label: "Total", val: totalAll }, { label: "To go", val: totalActive }, { label: "Done", val: totalDone }].map(function(s) {
          return (
      ...
      <div style={{ position: "relative", marginBottom: 8 }}>
        ...
        <input placeholder="Search destinations..." value={search} onChange={function(e) { setSearch(e.target.value); }}
        ...
        {search && <button onClick={function() { setSearch(""); }} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", fontSize: 18, color: "#888780", padding: 2, lineHeight: 1 }}>×</button>}
      </div>

      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: "0.75rem", alignItems: "center" }}>
        <select value={filterStatus} onChange={function(e) { setFilterStatus(e.target.value); }} style={{ fontFamily: "inherit" }}>
          <option value="active">Active</option><option value="complete">Completed</option><option value="all">All</option>
        </select>
        <select value={filterCat} onChange={function(e) { setFilterCat(e.target.value); }} style={{ fontFamily: "inherit" }}>
          <option value="All">All categories</option>
          {CATEGORIES.map(function(c) { return <option key={c}>{c}</option>; })}
        </select>
        <select value={filterCost} onChange={function(e) { setFilterCost(e.target.value); }} style={{ fontFamily: "inherit" }}>
          <option value="All">All costs</option>
          {COST_LEVELS.map(function(i) { return <option key={i} value={i}>{"$".repeat(i)} — {costLabels[i]}</option>; })}
        </select>
        <select value={filterTripLength} onChange={function(e) { setFilterTripLength(e.target.value); }} style={{ fontFamily: "inherit" }}>
          <option value="All">All lengths</option>
          {TRIP_LENGTHS.map(function(t) { return <option key={t}>{t}</option>; })}
        </select>
        <select value={filterKids} onChange={function(e) { setFilterKids(e.target.value); }} style={{ fontFamily: "inherit" }}>
          <option value="All">All trips</option><option value="yes">Kids trip</option><option value="no">Just us</option>
        </select>
        <select value={manualOrder ? "Manual" : sortBy} onChange={function(e) { if (e.target.value !== "Manual") { setManualOrder(null); setSortBy(e.target.value); } }} style={{ fontFamily: "inherit" }}>
          {SORT_OPTIONS.map(function(s) { return <option key={s}>{s}</option>; })}
          {manualOrder && <option value="Manual">Manual order</option>}
        </select>
        {manualOrder && <button onClick={function() { setManualOrder(null); }} style={{ fontSize: 12, fontFamily: "inherit" }}>Clear order</button>}
      </div>
        ...
        {sorted.map(function(item) {
          ...
          return (
            <div key={item.id} draggable
              onDragStart={function(e) { handleDragStart(e, item.id); e.currentTarget.style.opacity = "0.4"; }}
              onDragEnter={function(e) { handleDragEnter(e, item.id); }}
              ...
              <div onClick={function(e) { e.stopPropagation(); }} style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 3, width: 28, background: "#f0f0f2", borderRight: "1px solid rgba(0,0,0,0.07)", cursor: "grab", flexShrink: 0, padding: "0 6px" }}>
                {[0, 1, 2, 3, 4, 5].map(function(i) { return <div key={i} style={{ width: 12, height: 2, borderRadius: 1, background: "#b0b0b8" }} />; })}
              </div>
              <div style={{ width: 4, background: CAT_COLORS[item.category] || "#888780", flexShrink: 0 }} />
              <div onClick={function() { setError(null); setPanel({ item: item }); }} style={{ flex: 1, minWidth: 0, padding: "12px 14px", cursor: "pointer" }}>
              ...
              <div onClick={function() { setError(null); setPanel({ item: item }); }} style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", justifyContent: "center", gap: 5, padding: "12px 14px", flexShrink: 0, cursor: "pointer" }}>
